...
bool linker_link(LinkerConfig *cfg) {
    ...
    /* Patch _start's frame_size to total binary size only if the code
     * was generated by codegen_program (flat binary with built-in _start).
     * For user-defined _start (object output), leave it alone. */
    /* We detect this by checking if the first instruction at _start is
     * "push 0" (01 00 00 00 00) followed by ">r" (30) - the codegen_program pattern.
     * User-defined _start would typically have csr_write, a call, etc. */
    {
        int start_idx = gsym_lookup(&gt, "_start");
        if (start_idx >= 0 && gt.syms[start_idx].resolved) {
            uint32_t start_addr = gt.syms[start_idx].value;
            if (start_addr + 10 <= total_text_size &&
                merged[start_addr] == 0x01 &&
                merged[start_addr + 1] == 0x00 && merged[start_addr + 2] == 0x00 &&
                merged[start_addr + 3] == 0x00 && merged[start_addr + 4] == 0x00 &&
                merged[start_addr + 5] == 0x30) {
                /* This is the codegen_program _start pattern: push <zero>; >r */
